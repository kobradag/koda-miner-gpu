...
extern "C" {
    ...
    __global__ void heavy_hash(const uint64_t nonce_mask, const uint64_t nonce_fixed, const uint64_t nonces_len, uint8_t random_type, void* states, uint64_t *final_nonce) {
        ...
        if (nonceId < nonces_len) {
            ...
            uint8_t temp_result[32];  // Create a modifiable buffer
            memcpy(temp_result, result, 32);  // Copy SHA3 output

            heavy_hash_step(temp_result, output_hash);  

            uint256_t temp_hash;
            memcpy(temp_hash.hash, output_hash, HASH_SIZE); // Ensure SHA3 result is assigned properly

            convert_endian((uint32_t*)temp_hash.hash, hash_.hash); // Convert into `hash_`
            hash_ = temp_hash;  // Explicitly assign final processed hash to `hash_`*/
