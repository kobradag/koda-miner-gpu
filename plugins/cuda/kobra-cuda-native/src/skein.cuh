...
void cn_skein256_init(skeinHashState *state, size_t hashBitLen)
{
    ...
    // Ensure the IV is copied correctly
    memcpy(ctx->X, SKEIN_512_IV_256, 8 * sizeof(uint64_t));

    // Initialize tweak words correctly
    ctx->h.T[0] = 0;  // Initial tweak value
    ctx->h.T[1] = SKEIN_T1_FLAG_FIRST | SKEIN_T1_BLK_TYPE_MSG;
    ctx->h.bCnt = 0; // Reset byte counter
