...
use crate::proto::kobrad_message::Payload;
use crate::proto::rpc_client::RpcClient;
use crate::proto::{
    GetBlockTemplateRequestMessage, GetInfoRequestMessage, KobradMessage, NotifyBlockAddedRequestMessage,
...
type BlockHandle = JoinHandle<Result<(), PollSendError<KobradMessage>>>;

#[allow(dead_code)]
pub struct KobradHandler {
    client: RpcClient<TonicChannel>,
    pub send_channel: Sender<KobradMessage>,
    stream: Streaming<KobradMessage>,
    ...
    async fn listen(&mut self, miner: &mut MinerManager) -> Result<(), Error> {
        while let Some(msg) = self.stream.message().await? {
            match msg.payload {
                ...
                None => warn!("kobra message payload is empty"),
            }
        }
        Ok(())
    }
...
impl KobradHandler {
    ...
    fn create_block_channel(send_channel: Sender<KobradMessage>) -> (Sender<BlockSeed>, BlockHandle) {
        // KobradMessage::submit_block(block)
            ...
            tokio::spawn(async move {
                ReceiverStream::new(recv)
                    .map(|block_seed| match block_seed {
                        FullBlock(block) => KobradMessage::submit_block(*block),
    ...
    async fn client_send(&self, msg: impl Into<KobradMessage>) -> Result<(), SendError<KobradMessage>> {
        self.send_channel.send(msg.into()).await
    }

    async fn client_get_block_template(&mut self) -> Result<(), SendError<KobradMessage>> {
    ...
    async fn handle_message(&mut self, msg: Payload, miner: &mut MinerManager) -> Result<(), Error> {
        match msg {
            ...
            Payload::SubmitBlockResponse(res) => match res.error {
                None => info!("{}", "block submitted successfully!".green()),
            ...
            Payload::GetInfoResponse(info) => {
                info!("Kobrad version: {}", info.server_version);
                let kobra_version = Version::parse(&info.server_version)?;
                let update_version = Version::parse(VERSION_UPDATE)?;
                match kobra_version >= update_version {
            ...
            Payload::NotifyBlockAddedResponse(res) => match res.error {
                None => info!("Registered for block notifications (upgrade your Kobrad for better experience)"),
