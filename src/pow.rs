        ...
        match self {
            BlockSeed::FullBlock(block) => {
                ...
                info!(
                    "{} {}",
                    "Found a block:".green(),
                    format!("{:x} (Timestamp: {})", block_hash, block_time.format(format).unwrap_or_else(|_| "unknown".to_string()))
                );
            }
...
impl State {
    ...
    pub fn new(id: usize, block_seed: BlockSeed) -> Result<Self, Error> {
        ...
        hasher.update(&header_timestamp.to_le_bytes());
    ...
    pub fn calculate_pow(&self, nonce: u64) -> Uint256 {
        ...
        let mut blake2_hasher = blake2b_simd::Params::new().hash_length(32).to_state();
        blake2_hasher.update(hash.as_bytes());
        let blake2_hash = blake2_hasher.finalize();
        
        // 2. Skein hashing
        let mut skein_hasher = Skein512::<U32>::new();
        skein_hasher.update(blake2_hash.as_bytes());
        let skein_hash = skein_hasher.finalize();
   
        // 3. SHA3-256 hashing
        let mut sha3_hasher = Sha3_256::new();
        sha3_hasher.update(skein_hash);
        let sha3_hash = sha3_hasher.finalize();
    
        // Convert to Uint256
        let sha3_hash_bytes: [u8; 32] = sha3_hash.as_slice().try_into().expect("SHA3 output length mismatch");
